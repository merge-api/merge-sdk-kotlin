...
try {
    val result : PaginatedJournalEntryList = apiInstance.journalEntriesList(companyId, createdAfter, createdBefore, cursor, expand, includeDeletedData, includeRemoteData, modifiedAfter, modifiedBefore, pageSize, remoteId, transactionDateAfter, transactionDateBefore)
    println(result)
} catch (e: ClientException) {
