...
# **journalEntriesList**
> PaginatedJournalEntryList journalEntriesList(createdAfter, createdBefore, cursor, includeDeletedData, includeRemoteData, modifiedAfter, modifiedBefore, pageSize, remoteId)
...
try {
    val result : PaginatedJournalEntryList = apiInstance.journalEntriesList(createdAfter, createdBefore, cursor, includeDeletedData, includeRemoteData, modifiedAfter, modifiedBefore, pageSize, remoteId)
    println(result)
} catch (e: ClientException) {
...
> JournalEntry journalEntriesRetrieve(id, includeRemoteData)
...
val includeRemoteData : kotlin.Boolean = true // kotlin.Boolean | Whether to include the original data Merge fetched from the third-party to produce these models.
try {
    val result : JournalEntry = apiInstance.journalEntriesRetrieve(id, includeRemoteData)
    println(result)
} catch (e: ClientException) {
